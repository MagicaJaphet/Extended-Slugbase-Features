...
namespace ExtendedSlugbase.Objects
{
    public class SlugbaseObjects
    {
        public class AtlasManager
        {
            ...
            public static Dictionary<SlugBaseCharacter, Dictionary<SpriteElement, string>> SpriteElements = [];

            public enum SpriteElement
            {
                JollyIcon, 
                JollyIconDead,
                JollyPlayerIcon,
            ...
            public static string GetNameKey(SpriteElement element, SlugBaseCharacter slugcat)
            {
                var name = GetSlugbaseName(slugcat);
                return element switch
                {
                    SpriteElement.JollyIcon => $"jolly_icon_{name}",
                    SpriteElement.JollyIconDead => $"jolly_icon_{name}_dead",
                    SpriteElement.JollyPlayerIcon => $"{name}_pup_off",
                    SpriteElement.JollyPlayerUniqueIcon => $"unique_{name}_pup_off",
                    _ => null,
                };
            }

            public static bool TryGetElement(SlugBaseCharacter character, SpriteElement element, out string result, string folder = "illustrations")
            {
                result = null;
            ...
            internal static void LoadSlugbaseImages()
            {
                foreach (var slugcat in SlugBaseCharacter.Registry.Values)
                {
                    ...
                    foreach(SpriteElement element in Enum.GetValues(typeof(SpriteElement)))
                    {
