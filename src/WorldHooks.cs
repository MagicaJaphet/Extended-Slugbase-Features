		...
		public class WorldHooks
		{
					...
					foreach (Type type in roomDict.Keys)
					{
						...
						else if (type == typeof(AbstractPhysicalObject))
						{
							...
							if (objectCanidates != null && objectCanidates is Dictionary<AbstractPhysicalObject.AbstractObjectType, Dictionary<string, object>> objectDict)
							{
								...
								{
									AbstractPhysicalObject obj = Resources.ParseDictToObject(room, objType, objectDict[objType]);
			...
			private static void Ghost_Update(ILContext il)
			{
				try
				{
					...
					{
						return isSlugcat || self.room.game.HasFeature(Resources.spirituallyEnlightened);
					}
					...
					cursor.MoveCursorToNextSlugcatInstance(x => x.MatchLdsfld<MoreSlugcatsEnums.SlugcatStatsName>(nameof(MoreSlugcatsEnums.SlugcatStatsName.Saint)));

					cursor.Emit(OpCodes.Ldarg_0);
					cursor.EmitDelegate(CanTalkToGhosts);

					// if (this.room.game.session is StoryGameSession && ((this.room.game.session as StoryGameSession).saveState.deathPersistentSaveData.theMark || (ModManager.MSC && this.room.game.StoryCharacter == MoreSlugcatsEnums.SlugcatStatsName.Saint) || (ModManager.Watcher && this.room.game.StoryCharacter == WatcherEnums.SlugcatStatsName.Watcher)))
					cursor.MoveCursorToNextSlugcatInstance(x => x.MatchLdsfld<WatcherEnums.SlugcatStatsName>(nameof(WatcherEnums.SlugcatStatsName.Watcher)));
			...
			{
				return orig(save) || Custom.rainWorld.processManager.currentMainLoop is RainWorldGame game && game.HasFeature(Resources.spirituallyEnlightened);
			}
			...
			internal static void Room_Loaded(ILContext il)
			{
				try
				{
						...
						))
					{
						...
						{
							return !Resources.spawnBroadcasts.TryGet(self.game, out var slug) || !slug;
						}
					...
					cursor.MoveCursorToNextSlugcatInstance(x => x.MatchLdsfld<SlugcatStats.Name>(nameof(SlugcatStats.Name.Red)));

					cursor.Emit(OpCodes.Ldarg_0);
					static bool DontSpawnKarmaFlowers(bool isRed, Room self)
					{
						return isRed && (!Resources.canSpawnKarmaFlowers.TryGet(self.game, out bool canSpawn) || canSpawn);
					}
			...
			{
				if (ModManager.MSC && self.game != null && Resources.maxSlugpups.TryGet(self.game, out int maxPups))
				{
					return maxPups;
				}
